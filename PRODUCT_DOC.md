...
1. **Daily school scan:** Automatically visits each supported school source with OpenAI Codex CLI, records newly posted Open Day / Information Day notices, and surfaces the last-checked timestamp for every school so parents see current coverage.
...
5. **Admin monitoring:** Internal console monitors crawler health (including failure logs), missing data, and high-latency sources so the team can intervene before parents notice gaps.
